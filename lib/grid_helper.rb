...
# Methods that do not directly interact with the grid
  ...
  def self.validate_small_ship(start_row_index, start_column_index, end_row_index, end_column_index, user)
  ...
  def self.validate_large_ship(start_row_index, start_column_index, end_row_index, end_column_index, user)
    if ship_is_diagonal?(start_row_index, start_column_index, end_row_index, end_column_index)
      puts "You tried placing a ship diagonally! Your ship can only be horizontal or vertical. Please try again." if user == :player
      return false
  ...
  def self.small_ship_wraps_or_is_too_long?(start_row_index, start_column_index, end_row_index, end_column_index)
    ( end_column_index - start_column_index ) > 1 || ( end_row_index - start_row_index ) > 1
  end

  def self.large_ship_wraps_or_is_too_long?(start_row_index, start_column_index, end_row_index, end_column_index)
    ( end_column_index - start_column_index ) > 2 || ( end_row_index - start_row_index ) > 2 
  end

  def self.large_ship_is_too_small(start_row_index, start_column_index, end_row_index, end_column_index)
    ( end_column_index - start_column_index ) == 1 || ( end_row_index - start_row_index ) == 1
  end
